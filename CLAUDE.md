...
- **`editor/`** — editor subsystem:
  ...
  - **`mixed`** — mixed inline editor (`MixedEditor`): every segment renders via `TypstEngine` and flips to a monospace source `TextEdit` on click; hit-tests clicks against per-segment link rectangles and emits an `Option<String>` nav target; suppresses egui's full-row caret and paints its own blinking font-sized one; owns the dirty flag and `EditorConfig`. Borrows `&mut RenderCache` from `App` rather than owning the cache itself.
  - **`render_cache`** — `RenderCache`: byte-budget LRU for compiled `RenderedPage` textures and failed-compile error strings, keyed on the fully wrapped Typst source. Owned by `App` so cached textures survive note switches and cross-note duplicate segments hit the cache. `get`/`get_failed` bump per-entry `last_used` stamps; `insert` evicts the min-stamp entry until `bytes_in_use ≤ byte_budget`. Defaults: `DEFAULT_BYTE_BUDGET = 256 MB`, failed map capped at `FAILED_CAP = 1024` entries.
...
4. Each frame, `ensure_rendered` compiles uncached segments within a 16 ms budget; compiled segments enter `App.render_cache` as `RenderedPage { texture, links }`, the rest show `⟳ rendering…` and are compiled on the next repaint. The cache is shared across notes, so switching notes does not invalidate entries — they age out only when LRU eviction kicks in
...
`App.render_cache: RenderCache` holds `RenderedPage` values (texture + link rectangles) keyed by the *effective source* (template + preamble + block body). Failed compiles are cached separately as `(String, last_used)` pairs to avoid retrying every frame. The cache lives on `App` and is borrowed as `&mut RenderCache` by `MixedEditor::show`, so it persists across note switches — a segment shared between notes (same heading, same preamble-only block, identical boilerplate) hits the cache directly.

Eviction is byte-budget LRU: each `RenderedPage` contributes `w * h * 4` bytes (RGBA) and the cache evicts the least-recently-used entry whenever an insert pushes `bytes_in_use` over `DEFAULT_BYTE_BUDGET` (256 MB). `get` and `get_failed` bump the entry's `last_used` stamp so on-screen segments stay hot. There is no per-frame "drop everything not in the current note" sweep — the cap is what bounds memory.

The failed-compile map has its own count cap (`FAILED_CAP = 1024`) and evicts its own oldest entry once full; the strings are tiny so this is effectively never hit in practice.
...
- The render cache key is the *fully wrapped* source (template + preamble + body), so changing any of those parts invalidates the entry. Failed compiles are tracked in the same `RenderCache` (separate map, count-capped at 1024) to avoid retrying every frame.
